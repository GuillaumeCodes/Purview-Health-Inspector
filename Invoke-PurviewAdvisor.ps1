...
$script:Rules = @(
    ...
    @{
        ...
        condition = @{
            ...
            assert = @{
                ...
                subject = 'the value Microsoft recommends'
    ...
    @{
        ...
        condition = @{
            ...
            assert = @{
                type = 'isNotEmpty'
                itemSingular = 'enforcing DLP policy'
                itemPlural = 'enforcing DLP policies'
            }
        }
    ...
    @{
        ...
        condition = @{
            ...
            assert = @{
                ...
                itemSingular = 'enforcing DLP policy'
                itemPlural = 'enforcing DLP policies'
                subject = 'the policy reaches the Devices location'
    ...
    @{
        ...
        condition = @{
            ...
            assert = @{
                type = 'isNotEmpty'
                itemSingular = 'retention label'
                itemPlural = 'retention labels'
            }
        }
...
function Invoke-PurviewCollector {
    ...
    param(
        ...
        [string]$RequiredRole = '',
        # A missing premium cmdlet can mean either licensing or role assignment. Only complete,
        # coherent licensing evidence may settle the first; otherwise the role-safe path remains.
    ...
    if ($missing.Count -gt 0) {
        ...
        if ($connected) {
            if ($null -ne $LicensingRequirement -and $null -ne $LicensingBlock) {
                $licensingSnapshot = [pscustomobject]@{ licensing = $LicensingBlock }
                $licensingState = Get-PurviewLicensingState -Licensing $LicensingRequirement -Snapshot $licensingSnapshot
                if ($licensingState.State -eq 'NotLicensed') {
                    $unlock = Format-PurviewGrantList -Grant @($licensingState.UnlockedBy)
                    $result.status = 'NotLicensed'
                    $result.errors = @([pscustomobject]@{
                            message = "$($licensingState.Capability) is not available because this tenant has no enabled license that provides it." +
                                $(if ($unlock) { " It would be unlocked by: $unlock." } else { '' })
                            category = 'NotLicensed'
                            interface = $Interface
                        })
                    $result.limitations = @("$SolutionArea was not read because no available tenant license provides this feature.")
...
function Get-PurviewProtectedFilesConsentData {
    ...
    return Invoke-PurviewCollector -Collector 'ProtectedFilesConsent' -SolutionArea 'PostureValidation' `
        ...
        # Microsoft publishes no app id for this principal, and it owns more first-party tenants
        # than any list here could keep up with, so neither identifies it. What does hold is that
        # Microsoft's application is never owned by the tenant being assessed: an application with
        # this name owned by this tenant was made here, and is not evidence of Microsoft's consent.
        $assessedTenant = ''
        if (Test-PurviewCommand -Name 'Get-MgContext') {
...
function Get-PurviewTenantSnapshot {
    ...
    # Licensing is read before the feature collectors: a premium cmdlet missing from a live
    # compliance session means either no license or no role, and only licensing evidence separates them.
...
function ConvertTo-PurviewFinding {
    ...
    if ($licensing.State -eq 'NotLicensed') {
        ...
        $unlock = Format-PurviewGrantList -Grant @($licensing.UnlockedBy)
...
function Get-PurviewLicensingAnalysis {
    ...
    if (-not $evidence.Collected) {
        $output.Add([pscustomobject]@{
                Sku = 'Not collected'
                State = 'Not read'
                Detail = 'This run did not read the tenant subscriptions, so nothing here says what is licensed. Signing in to Microsoft Graph includes them next time.'
            })
        return $output.ToArray()
    }
    ...
    if (-not $evidence.ListReadable) {
        $output.Add([pscustomobject]@{
                Sku = 'Not collected'
                State = 'Not read'
                Detail = 'The subscription list came back without any products, so nothing here says what is licensed.'
            })
        return $output.ToArray()
    }
    ...
    # Each active subscription is listed below in its own right, so a summary naming them again
    # would only repeat the table back to the reader. It appears where there is nothing to repeat.
    if ($evidence.BlockConflict -or $currentNames.Count -eq 0) {
        $detail = if ($evidence.BlockConflict) {
            'This run recorded the same subscription twice with details that disagree, so nothing is counted as available or unavailable.'
        }
        elseif ($evidence.CanProveExclusion -and $evidence.Entries.Count -eq 0) {
            'This tenant reports no subscriptions at all.'
        }
        elseif ($evidence.CanProveExclusion) {
            'No subscription that affects Purview is currently active.'
        }
        else {
            'The subscription details were incomplete, so what this tenant is entitled to could not be confirmed. Every check still runs rather than being skipped.'
        }

        $output.Add([pscustomobject]@{
                Sku = 'Purview entitlement'
    ...
    foreach ($entry in @($evidence.Entries | Where-Object { $_.Classification -eq 'Recognized' })) {
        ...
        if ($evidence.BlockConflict) {
            $output.Add([pscustomobject]@{
                    Sku = $productName
                    State = 'Unknown'
                    Detail = 'This run recorded this subscription more than once with details that disagree, so nothing is counted for or against it.'
                })
            continue
        }
        if ($productConflict) {
            $output.Add([pscustomobject]@{
                    Sku = $productName
                    State = 'Unknown'
                    Detail = 'This subscription was reported more than once, and the reports disagree, so nothing is counted for or against it.'
                })
            continue
        }
        if (-not $entry.Enabled) {
            $output.Add([pscustomobject]@{
                    Sku = $productName
                    State = if ($entry.StatusKnown) { [string]$entry.CapabilityStatus } else { 'Unknown' }
                    Detail = 'Nothing is counted as available through it, though the checks that rely on it are still assessed.'
                })
            continue
        }
        ...
        if ($entry.SeatCountsKnown) {
            ...
            # Only an imbalance earns a sentence. The counts are already in the status beside it,
            # and restating them there tells the reader something they can plainly see.
            $seatNote = if ($spare -gt 0) {
                "$(Format-PurviewCount -Count $spare -Singular 'seat') $(if ($spare -eq 1) { 'is' } else { 'are' }) paid for but unassigned."
            }
            elseif ($spare -lt 0) {
                "$(Format-PurviewCount -Count ([math]::Abs($spare)) -Singular 'more seat') $(if ($spare -eq -1) { 'is' } else { 'are' }) assigned than this subscription lists as available."
            }
            else { '' }
        }

        # Naming the subscription's own tier back to the reader would repeat the row label, so only
        # the lower tiers it also satisfies are worth stating.
        $alsoCovers = @($entry.Product.Grants |
                ForEach-Object { Get-PurviewGrantDisplayName -Grant ([string]$_) } |
                Where-Object { $_ -and $_ -ne $productName } | Select-Object -Unique)

        $detailParts = [System.Collections.Generic.List[string]]::new()
        if ($alsoCovers.Count -gt 0) {
            # A higher tier includes the lower one rather than depending on it, so this is stated as
            # what the subscription satisfies here. Any one of them satisfies a check on its own.
            $detailParts.Add(('Also satisfies the checks that need only {0}.' -f
                    (Join-PurviewPhrase -Item $alsoCovers -Conjunction 'or')))
        }
        if ($seatNote) { $detailParts.Add($seatNote) }

        # Nothing further to say is left blank rather than padded out.
        $output.Add([pscustomobject]@{
                Sku = $productName
                State = $seatState
...
function Get-PurviewRemediationPart {
    ...
    $header = @(
        ...
        '        throw "$Name could not be loaded, so nothing was changed."'
        '    }'
        '}'
        ''
...
function ConvertTo-PurviewHtmlReport {
    ...
    $null = $builder.AppendLine('<table><thead><tr><th>Subscription</th><th>Status</th><th>Detail</th></tr></thead><tbody>')
