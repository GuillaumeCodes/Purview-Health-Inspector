...
.DESCRIPTION
    ...
    evaluates it against built-in deterministic rules, and prints a summary with HTML and JSON
...
.PARAMETER Environment
    ...
    This selects connection defaults; it does not independently verify the cloud of existing sessions.
...
.PARAMETER SkipConnect
    ...
    You must ensure those sessions belong to the intended tenant in the supported commercial cloud;
    this script does not independently verify their cloud, including when reusing sessions normally.

.PARAMETER KeepSignedIn
    Leave the sessions this run opened signed in. By default it attempts to disconnect owned
    sessions. Cleanup does not guarantee token revocation or removal of authentication caches.
...
.PARAMETER RedactTenant
    ...
    review the output before sharing it outside the customer.
...
function Clear-PurviewStepScaffold {
    ...
    .DESCRIPTION
        Progress and its heading explain a long wait while it is happening, and say nothing once it
        is over. Removing them leaves one row per collector, reading the same as every other.
...
$script:Rules = @(
    @{
        ...
        severity = 'High'
        rationale = 'Label order affects selection and downgrade handling in supported scenarios, with higher sensitivity normally later in the list. This check flags duplicate priority values for review; it does not model all precedence rules or same-parent sublabel exceptions.'
    ...
    @{
        ...
        solutionArea = 'SensitivityLabels'
        severity = 'Medium'
        rationale = 'Duplicate display names can confuse users in the same label picker and in name-only reports. Sublabels are compared only against siblings: Confidential\All Employees is distinct from Highly Confidential\All Employees. Identifiers can distinguish labels in other views.'
...
$script:Prerequisite = @(
    ...
    @{
        ...
        Why = 'Supports manual labelling of MP4 files in SharePoint and OneDrive and supported Teams recording label inheritance. It does not establish that recordings are labelled; auto-labeling and default labelling do not apply to MP4 files.'
        Url = $script:DocUrl.SharePointLabelledFiles
    }
    @{
    ...
    @{
        ...
        Why = 'Enables co-authoring and AutoSave for eligible encrypted files in supported desktop and mobile Office apps. Eligible Office for the web collaboration is separate. Check metadata compatibility, app versions and encryption limitations before enabling.'
    ...
    @{
        ...
        Why = 'The modern scheme replaces parent labels with label groups and preserves applicable parent settings in sublabels during migration. A returned modern scheme does not establish how or when it was adopted. Review the migration guidance and compatibility before proceeding.'
...
function Get-PurviewSignInContext {
    <#
    .SYNOPSIS
        Reports the signed-in identity and observable direct Entra and Purview memberships.
    ...
    .DESCRIPTION
        ...
        distinct from an empty result. This does not evaluate effective access or PIM eligibility.
    #>
    [CmdletBinding()]
    [OutputType([pscustomobject])]
    param()
    ...
    if ((Test-PurviewCommand -Name 'Invoke-MgGraphRequest') -and (Test-PurviewConnected -Service 'Graph')) {
        ...
        try {
            ...
            $entraDetail = if ($entraUnnamed) { 'some direct role memberships were returned without readable names' } else { '' }
...
function Disconnect-PurviewSession {
    <#
    .SYNOPSIS
        Attempts module disconnects for recorded session ownership and removes owned compatibility resources.

    .DESCRIPTION
        Best-effort cleanup, not verified token revocation or cache removal. Module-wide disconnects
        may affect shared sessions; isolated Graph and Exchange-only ownership are not fully covered.
    #>
    [CmdletBinding()]
    param()
...
function Get-PurviewTaxonomyComparison {
    ...
    foreach ($sub in @($taxonomy.SubLabels | Sort-Object -Property Path)) {
        $detail = if ($null -eq $sub.Encrypted) { 'This label''s encryption configuration could not be established from the returned data.' }
...
function Get-PurviewInventory {
    ...
    $retentionQualifier = 'Configured policy kinds and states do not prove distribution or content application. No per-label policy association is established by this read.'
    ...
    if ($retentionEvidence.Complete -and $unknownKinds.Count -eq 0 -and $retention.Count -gt 0) {
        ...
        # Preserve the combined metric for older captures, but do not infer a total from an
        # untyped or partial set. Readable records remain useful, explicitly not a tenant total.
        $retentionValue = 'Not checked'
        ...
        elseif ($unknownKinds.Count -gt 0) {
            $retentionDetail += " RuleTypes was missing, malformed or unrecognized for $($unknownKinds.Count) returned named records; no enforcing total is inferred."
        }
        $policyDetails = @(foreach ($policy in @($retention | Select-Object -First 6)) {
                $parts = @([string](Get-PurviewProperty -InputObject $policy -Name 'Name'), (Get-PurviewRetentionPolicyKind -Policy $policy))
                foreach ($field in 'Enabled', 'Mode', 'HasRules') {
                    $rawState = $null
                    ...
                    $text = if ($null -ne $rawState -and ($rawState -is [string] -or $rawState -is [ValueType])) { [string]$rawState } else { 'unknown' }
                    $parts += "${field}=$text"
                }
                $parts -join '; '
            })
        if ($policyDetails.Count -gt 0) { $retentionDetail += " Returned named records (not a tenant total): $($policyDetails -join ' | ')." }
        if ($retention.Count -gt 6) { $retentionDetail += " $($retention.Count - 6) more returned named records omitted." }
        $output.Add([pscustomobject]@{
                Area = 'Data lifecycle'; Metric = 'Retention and retention label policies enforcing'; Collector = 'RetentionPolicy'
    ...
    foreach ($item in $script:Prerequisite) {
        ...
        elseif ($item.ContainsKey('LabelScheme')) {
            ...
            if ($reported -eq 'Modern') {
                $state = 'As recommended'
                $detail = 'The tenant reports the modern label scheme. This does not establish a migration history.'
            }
            ...
            elseif ($encrypting.Count -gt 0 -or $publishedGroups.Count -gt 0) {
                ...
                $detail = 'Labels are organised in two tiers, but this read does not distinguish parent labels from label groups. Confirm the scheme separately.'
            }
        }
...
function Get-PurviewRemediationPart {
    ...
    $header = @(
        ...
        '    This script does not independently verify the cloud of existing sessions.'
        '    Ensure all sessions belong to the intended tenant in the supported commercial cloud.'
        ''
        ...
        '    # Left to autoload, a module that will not load reports only that the command was not'
        '    # found. Importing it here is what puts the real reason, and the way out, on screen.'
        '    try { Import-Module -Name $Name -ErrorAction Stop }'
        '    catch {'
        '        Write-Host ""'
        '        Write-Host "$Name could not be loaded in this PowerShell." -ForegroundColor Yellow'
        '        Write-Host "  $($_.Exception.Message)" -ForegroundColor Yellow'
        '        Write-Host "  This script was written by a PowerShell 7 run, so run it in PowerShell 7:" -ForegroundColor Yellow'
        '        Write-Host "    pwsh -NoProfile -File ""$PSCommandPath""" -ForegroundColor Yellow'
        '        throw "$Name could not be loaded; this step stopped. Earlier steps may have made changes."'
        '    }'
        '}'
        ''
    ...
    $connect = [ordered]@{
        SharePoint = @(
            ...
            '    else { Import-PurviewRemediationModule -Name ''Microsoft.Online.SharePoint.PowerShell'' }'
            '}'
            ...
            'else {'
            '    # Prefer system-browser authentication. The fallback may change the available'
            '    # authentication methods; this catch does not identify why the first attempt failed.'
    ...
    foreach ($item in @($Prerequisite | Where-Object { ($_.State -eq 'Needs attention' -and ($_.Command -or $null -ne $_.Script)) -or @($_.Candidates).Count -gt 0 })) {
        ...
        if ($null -ne $item.Script) {
            ...
            # This workflow leaves migration to the portal. It records parent-label publishing
            # pairs before migration and offers to unpublish matching sublabels afterward.
            ...
            # This displayed command can be copied by the operator; quote CSV values too.
            $lines.Add('        $quotedPolicy = [System.Management.Automation.Language.CodeGeneration]::EscapeSingleQuotedStringContent([string]$row.Policy)')
            $lines.Add('        $quotedLabel = [System.Management.Automation.Language.CodeGeneration]::EscapeSingleQuotedStringContent([string]$row.ParentLabel)')
            $lines.Add('        $cmd = "Set-LabelPolicy -Identity ''{0}'' -RemoveLabels ''{1}''" -f $quotedPolicy, $quotedLabel')
            ...
            $lines.Add('    Write-Host ("Recorded {0} publishing pairs to {1}. This recording step did not change tenant configuration." -f @($record).Count, $path)')
...
function Write-PurviewSecretArgument {
    ...
    .DESCRIPTION
        ...
        collector happened to reclaim it. Owned character/byte arrays and the BSTR are cleared on
        exit; copies in stream buffers, the runtime or qpdf are outside this cleanup guarantee.
    #>
    [CmdletBinding()]
    param(
...
function Protect-PurviewPdf {
    ...
    try {
        ...
        # WaitForExit is bounded, but the preceding stream reads can block before that wait.
        # Exit status and file existence are checked before replacement, not full PDF validity.
...
function Test-PurviewEvidence {
    ...
    .DESCRIPTION
        ...
        as such rather than assumed current. This does not check every URL in the script or prove
        that a source supports the associated claim.
    #>
    [CmdletBinding()]
    [OutputType([pscustomobject[]])]
    param(
...
function Import-PurviewRuleSet {
    ...
    foreach ($rule in @(ConvertTo-PurviewArray -InputObject (Get-PurviewProperty -InputObject $document -Name 'rules'))) {
        ...
        if ($analysis) {
            if ($analysis -notin 'EndpointCoverage', 'PolicyRuleCoverage', 'DisabledRulesInEnforcingPolicies', 'RetentionLabelConfiguration') {
...
function ConvertTo-PurviewHtmlReport {
    ...
    $null = $builder.AppendLine('<table><thead><tr><th>Area</th><th>Measure</th><th>Value</th><th>Detail</th></tr></thead><tbody>')
    ...
    if ($subscriptions.Count -gt 0) {
        $null = $builder.AppendLine('<table class="licensing-table"><thead><tr><th scope="col">Subscription</th><th scope="col">Status</th><th scope="col" class="seats">Enabled seats</th><th scope="col" class="seats">Assigned seats</th></tr></thead><tbody>')
    ...
    if ($allSubscriptions.Count -gt 0) {
        ...
        $null = $builder.AppendLine('<details class="licensing-technical"><summary>Technical details</summary><p class="dim">All returned records, including deleted and unrecognized products. Retained service-plan fields are in snapshot.json under licensing.subscribedSkus[].servicePlans. Technical details are omitted from print.</p><table><thead><tr><th scope="col">Subscription</th><th scope="col">Status</th><th scope="col">Observed details</th></tr></thead><tbody>')
        foreach ($row in $allSubscriptions) { Add-Row @((Enc $row.Sku), (Enc $row.State), (Enc $row.Detail)) }
        $null = $builder.AppendLine('</tbody></table></details>')
    }
            ...
            [pscustomobject]@{
                ...
                data = [pscustomobject]@{
                    Labels = @(
                        ...
                        [pscustomobject]@{ Guid = '76ca4345-a8dc-490b-a9a0-4c33ff409610'; Name = 'Confidential'; UniqueName = '76ca4345-a8dc-490b-a9a0-4c33ff409610'; ParentId = $null; Priority = 2; Disabled = $false; ContentType = 'File, Email'; EncryptionEnabled = $false }
                        ...
                        [pscustomobject]@{ Guid = '46a0e922-db10-4480-a8ba-310a12e86296'; Name = 'Highly Confidential'; UniqueName = '46a0e922-db10-4480-a8ba-310a12e86296'; ParentId = $null; Priority = 2; Disabled = $false; ContentType = 'File, Email'; EncryptionEnabled = $true; EncryptionRights = 'VIEW, EDIT' }
                        [pscustomobject]@{ Guid = 'dca36009-c6fa-49ae-b4f5-1c42fabdcfea'; Name = 'Board Material'; UniqueName = 'dca36009-c6fa-49ae-b4f5-1c42fabdcfea'; ParentId = $null; Priority = 3; Disabled = $false; ContentType = 'File, Email'; EncryptionEnabled = $false }
                        ...
                        [pscustomobject]@{ Guid = '37e88e8a-6806-4936-85e4-77eeae1e27be'; Name = 'Highly Confidential \ Specific People'; UniqueName = '37e88e8a-6806-4936-85e4-77eeae1e27be'; ParentId = '46a0e922-db10-4480-a8ba-310a12e86296'; Priority = 5; Disabled = $false; ContentType = 'File, Email'; EncryptionEnabled = $true; EncryptionRights = 'VIEW, EDIT' }
                        [pscustomobject]@{ Guid = '1854dd97-5860-4af7-87e0-46e2d729e822'; Name = 'General \ Anyone (unrestricted)'; UniqueName = '1854dd97-5860-4af7-87e0-46e2d729e822'; ParentId = '8c629e92-39dc-4f78-baf3-e31522b06dc9'; Priority = 6; Disabled = $false; ContentType = 'File, Email'; EncryptionEnabled = $false }
                    )
                }
                errors = @(); limitations = @()
            }
...
function Show-SignIn {
    ...
    if (-not $entraDetail -and [bool](Get-PurviewProperty -InputObject $Context -Name 'EntraRoleUnnamed')) {
        $entraDetail = 'some direct role memberships were returned without readable names'
    }
    if (-not $entraDetail -and -not $Context.EntraRoleRead) { $entraDetail = 'not read; direct Entra memberships could not be determined' }
    $entra = if (@($Context.EntraRole).Count -gt 0) { @($Context.EntraRole) -join ', ' }
    elseif ($Context.EntraRoleRead) { 'none held directly' }
    else { '' }
    ...
    Write-Line -Message ''
    Write-Line -Style Dim -Message '    Direct memberships only; not an effective-permissions or PIM eligibility audit.'
}
    ...
    if (-not $ProtectPdf -or $null -ne $secret) {
        ...
        if ($pdf -and $ProtectPdf) {
            if (Protect-PurviewPdf -PdfPath $pdf -Password $secret -QpdfPath $qpdfPath) {
                ...
                # Best-effort removal; failure is suppressed, so deletion is not verified.
                Remove-Item -LiteralPath $pdf -Force -ErrorAction SilentlyContinue
                Write-Line -Style Bad -Message '    PDF encryption failed and deletion was attempted. Verify the output path before sharing.'
            }
        }
...
if ($PassThru) { $findings }
    ...
    if (-not $KeepSignedIn -and $signedIn) {
        Write-Line -Style Dim -Message '  Cleanup attempted for sessions opened by this run; token revocation and cache removal are not verified.'
        Write-Line -Message ''
    }
}
