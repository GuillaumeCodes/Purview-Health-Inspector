...
| `NotLicensed` | The feature is unavailable because no enabled tenant license provides it |
...
Each rule cites the specific Microsoft page it was written against and the date that page was read.
The rules are judgement calls informed by that guidance, not something extracted from it
automatically — no tool can turn prose like "labels need distinct priorities" into a check on its
own.
...
**Rules are customizable.** Nothing about them is fixed in the script:
...
operators, rules that do not say which collector they read, and citations that resolve to nothing
are all rejected, and any cited source must be `https`.
